  ...
  constructor(
    ...
    ConfigFetcher().then((conf) => {
      ...
      if (this.config.spotifyClientId && this.config.spotifyClientSecret) {
        this.warn("Spotify credentials detected ✅");
      } else {
        this.warn("Spotify credentials MISSING ❌ - Spotify links will NOT work.");
      }
      this.build();
    });
  ...
  build() {
    ...
    this.manager = new Manager({
      ...
      .on("trackError", (player, err) => {
        ...
        console.log(`Full Track Error: ${JSON.stringify(err, null, 2)}`); // Debugging 'undefined' errors
