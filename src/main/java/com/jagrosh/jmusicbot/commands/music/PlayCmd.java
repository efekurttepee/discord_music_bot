...
import pw.chew.jdautilities.command.Command;
import pw.chew.jdautilities.command.CommandEvent;
import pw.chew.jdautilities.menu.ButtonMenu;
...
public class PlayCmd extends MusicCommand {
    ...
    public PlayCmd(Bot bot) {
        ...
        this.help = "belirtilen şarkıyı çalar";
    ...
    public void doCommand(CommandEvent event) {
        if (event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            ...
            StringBuilder builder = new StringBuilder(event.getClient().getWarning() + " Çalma Komutları:\n");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name)
                    .append(" <şarkı başlığı>` - Youtube'dan ilk sonucu çalar");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name)
                    .append(" <URL>` - belirtilen şarkıyı, listeyi veya yayını çalar");
        ...
        event.reply(loadingEmoji + " Yükleniyor... `[" + args + "]`", m -> bot.getPlayerManager()
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess() + " Eklendi: **" + track.getInfo().title
                    + "** (`" + TimeUtil.formatTime(track.getDuration()) + "`) "
                    + (pos == 0 ? "çalmaya başlıyor" : " sıraya eklendi. Konum: " + pos));
            if (playlist == null
                    || !event.getSelfMember().hasPermission(event.getTextChannel(), Permission.MESSAGE_ADD_REACTION))
                m.editMessage(addMsg).queue();
            else {
                new ButtonMenu.Builder()
                        .setText(addMsg + "\n" + event.getClient().getWarning() + " Bu parçanın yanında **"
                                + playlist.getTracks().size() + "** parçalık bir liste var. Listeyi yüklemek için "
                                + LOAD + " seçin.")
                        ...
                        .setAction(re -> {
                            if (re.getName().equals(LOAD))
                                m.editMessage(addMsg + "\n" + event.getClient().getSuccess() + " **"
                                        + loadPlaylist(playlist, track) + "** ek parça yüklendi!").queue();
                            else
                                m.editMessage(addMsg).queue();
                        }).setFinalAction(m -> {
                            try {
        ...
        public void noMatches() {
            if (ytsearch)
                m.editMessage(FormatUtil
                        .filter(event.getClient().getWarning() + " `" + event.getArgs() + "` için sonuç bulunamadı."))
                        .queue();
            else
